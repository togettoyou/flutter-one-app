...
class Api {
  ...
  static String getToolBarList(String date) {
    return "$_baseUrl/feeds/list/$date$_endUrl";
  }

  ///获取ALL页面专题列表(Banner)
  static final String allBannerUrl =
      "$_baseUrl/banner/list/4?last_id=0$_endUrl";

  ///获取ALL页面近期热门作者列表
  static final String hotAuthorUrl = "$_baseUrl/author/hot$_endUrl";

  ///获取热门作者动态，page_num当前页数，author_id作者ID
  static String getHotAuthorWorksUrl(int num, String id) {
    return "$_baseUrl/author/works$_endUrl&page_num=$num&author_id=$id";
  }

  ///获取特定日期的图文信息
  static String getCategoryUrl(String type, String date) {
    return "$_baseUrl/$type/bymonth/$date$_endUrl";
  }

  ///获取文章详情
  static String getDetailsUrl(String type, String id) {
    return "$_baseUrl/$type/detail/$id$_endUrl";
  }

  ///获取影视文章详情
  static String getDetailsByMovieUrl(String id) {
    return "$_baseUrl/movie/$id/story/1/0$_endUrl";
  }

  ///获取文章评论
  static String getCommentUrl(String type, String id) {
    return "$_baseUrl/comment/praiseandtime/$type/$id/0$_endUrl";
  }

  ///获取ALL页面专题列表(Banner)详情
  static String getDetailsByAllBanner(String id) {
    return "$_baseUrl/topic/htmlcontent/$id$_endUrl";
  }
}
