...
/// [MySearchDelegate.close] call. If the search page is closed with the system
/// back button, it returns null.
///
/// A given [MySearchDelegate] can only be associated with one active [showSearch]
/// call. Call [MySearchDelegate.close] before re-using the same delegate instance
...
/// The animation provided by [MySearchDelegate.transitionAnimation] can be used
...
/// See also:
///
///  * [MySearchDelegate] to define the content of the search page.
...
/// query text field can be customized via [MySearchDelegate.leading] and
/// [MySearchDelegate.actions].
///
/// The body below the [AppBar] can either show suggested queries (returned by
/// [MySearchDelegate.buildSuggestions]) or - once the user submits a search  - the
/// results of the search as returned by [MySearchDelegate.buildResults].
///
/// [MySearchDelegate.query] always contains the current query entered by the user
/// and should be used to build the suggestions and results.
///
/// The results can be brought on screen by calling [MySearchDelegate.showResults]
/// and you can go back to showing the suggestions by calling
/// [MySearchDelegate.showSuggestions].
///
/// Once the user has selected a search result, [MyMySearchDelegate.close] should be
...
abstract class MySearchDelegate<T> {
  ...
  /// {@tool sample}
  /// ```dart
  /// class CustomSearchHintDelegate extends MySearchDelegate {
                ...
                decoration: InputDecoration(
                  ...
                  hintStyle: TextStyle(
                    ...
                    color: Color.fromRGBO(0, 0, 0, 0.87).withOpacity(
                      Color.fromRGBO(0, 0, 0, 0.87).opacity * 0.5,
                    ),
                  ),
                ),
              ),
